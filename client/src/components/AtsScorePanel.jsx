...
const ScoreGauge = ({ score, accentColor = "#6366f1" }) => {
    ...
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-4xl font-black font-heading text-slate-900 leading-none">{animatedScore}</span>
...
const CategoryRow = ({ category, isOpen, onToggle, accentColor = "#6366f1" }) => {
  return (
      ...
      <button onClick={onToggle} className="w-full px-6 py-5 flex items-center justify-between hover:bg-slate-50/80 transition-colors">
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">{category.name}</h4>
            <div className="flex items-center gap-2 mt-1">
               <div className="w-24 h-1 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full" style={{ width: `${category.score}%`, backgroundColor: accentColor }} />
               </div>
               <span className="text-[10px] font-black text-slate-400 uppercase">{category.score}%</span>
            </div>
          </div>
        </div>
...
const AtsScorePanel = ({ resumeData, accentColor = "#6366f1" }) => {
  ...
  const checkAtsScore = async () => {
    setLoading(true)
    try {
      const response = await api.post('/api/ai/check-ats-score', { resumeData }, {
  ...
  return (
    <div className="glass-light rounded-[2.5rem] border border-slate-300/60 shadow-xl overflow-hidden mt-8">
      <div className="p-8 border-b border-slate-50 flex items-center justify-between">
        <div className="flex items-center gap-4">
           <div className="size-10 rounded-xl flex items-center justify-center bg-white shadow-sm border border-slate-100" style={accentStyle}>
              <Target size={20} />
           </div>
           <div>
              <h3 className="text-slate-900 font-black text-lg uppercase tracking-tight">ATS Score Checker</h3>
           </div>
        </div>
        <button 
          onClick={checkAtsScore} 
          disabled={loading} 
          className="px-6 h-10 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg flex items-center gap-2 disabled:opacity-50"
          style={{ ...accentBgStyle, boxShadow: `0 10px 20px -10px ${accentColor}40` }}
        >
          {loading ? <Loader2 size={14} className="animate-spin" /> : <Zap size={14} />}
          {loading ? "Analyzing..." : (checked ? "Re-Check" : "Check Score")}
        </button>
      </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="relative">
              <Loader2 size={48} className="animate-spin" style={accentStyle} />
              <div className="absolute inset-0 flex items-center justify-center">
                 <div className="size-2 rounded-full animate-pulse" style={accentBgStyle} />
              </div>
            </div>
            <p className="mt-8 text-[11px] font-black text-slate-900 uppercase tracking-[0.4em] animate-pulse">Analyzing Resume Data...</p>
          </div>
        ) : scoreData ? (
          <div className="space-y-10">
            <ScoreGauge score={scoreData.overall_score || scoreData.score} accentColor={accentColor} />
            ...
            <div className="text-center px-4 -mt-4">
               ...
               {scoreData.total_issues !== undefined && (
                  <div className="inline-flex items-center gap-2 mt-4 px-3 py-1 bg-red-50 text-red-500 rounded-full">
                     <XCircle size={12} />
                     <span className="text-[9px] font-black uppercase tracking-widest">{scoreData.total_issues} Optimization Tips</span>
                  </div>
               )}
            </div>
            ...
            <div className="p-6 bg-slate-50 border border-slate-200 rounded-3xl flex items-start gap-4">
               <Zap size={14} className="mt-0.5" style={accentStyle} />
               <p className="text-[11px] font-bold text-slate-600 leading-relaxed uppercase tracking-wider">
                  Optimizer Tip: Resolving "Critical" issues first can boost your visibility score by up to 40%.
               </p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            <div className="size-20 rounded-full bg-slate-50 flex items-center justify-center mb-6">
              <Target size={32} className="text-slate-300" strokeWidth={1} />
            </div>
            <p className="text-sm font-bold text-slate-900">Analysis Ready</p>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 max-w-[200px] leading-relaxed">
