    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col items-center gap-4 py-4">
        <label className="relative group cursor-pointer">
           <div className="size-24 rounded-3xl bg-slate-50 border-2 border-white shadow-xl flex items-center justify-center overflow-hidden relative transition-all group-hover:scale-105 active:scale-95">
              {data.image ? (
                <>
                   <img 
        ...
        <span className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] opacity-40">Profile Image</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        {fields.map((field) => (
            <motion.div 
              ...
              className={`space-y-2 ${field.key === 'full_name' || field.key === 'email' ? 'md:col-span-2' : ''}`}
            >
                <label className="flex items-center gap-2 text-[10px] font-black text-secondary uppercase tracking-widest pl-1">
                    <field.icon size={12} className="text-accent" />
                    {field.label}
                ...
                <input 
                  ...
                  className="w-full bg-white border border-slate-100 rounded-2xl px-5 py-4 text-sm font-medium text-primary placeholder:text-slate-300 focus:border-accent/30 focus:ring-4 focus:ring-accent/5 outline-none transition-all shadow-sm"
